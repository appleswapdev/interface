...
import { getEtherscanLink, isAddress, translateName } from '../../utils'
      ...
      <AutoRow style={{ justifyContent: 'flex-start', width: 'fit-content' }}>
        ...
        <div style={{ marginLeft: '.75rem' }}>
          <TYPE.mediumHeader>
            ...
            {chainId && token.equals(WETH[chainId]) ? 'WETH' : token.symbol}/{translateName('ETH')}
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        ...
        {chainId && (
          <ExternalLink href={getEtherscanLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">Appleswap migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            It{"'"}s best to deposit liquidity into Appleswap V2 at a price you believe is correct. If the V2 price seems
          ...
          <AutoColumn gap="8px">
            <RowBetween>
              ...
              <TYPE.black>
                {v1SpotPrice?.toSignificant(6)} {token.symbol}/{translateName('ETH')}
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v1SpotPrice?.invert()?.toSignificant(6)} {translateName('ETH')}/{token.symbol}
              </TYPE.black>
            </RowBetween>
            ...
            <RowBetween>
              ...
              <TYPE.black>
                {v2SpotPrice?.toSignificant(6)} {token.symbol}/{translateName('ETH')}
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v2SpotPrice?.invert()?.toSignificant(6)} {translateName('ETH')}/{token.symbol}
              </TYPE.black>
            </RowBetween>
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            You are the first liquidity provider for this pair on Appleswap V2. Your liquidity will be migrated at the
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your Appleswap V1 ${token.symbol}/${translateName('ETH')} liquidity will become Appleswap V2 ${token.symbol}/${translateName('ETH')} liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `UNI-V1-${token.symbol}`, 'Appleswap V1')
        : undefined,
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <div>
            <QuestionHelper text="Migrate your liquidity tokens from Appleswap V1 to Appleswap V2." />
          </div>
        </AutoRow>
        ...
        {!account ? (
          ...
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because Appleswap V2 uses WETH under the hood, your Appleswap V1 WETH/{translateName('ETH')} liquidity cannot be migrated. You
